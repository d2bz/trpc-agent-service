...
2026-09-07 网页聊天和企业微信智能机器人长连接（Bot ID + Secret）的文本链路均已完成。网页启动后可打开 `/` 发送、续聊和停止，见[网页切片验收](acceptance.md#网页切片验收2026-09-07)；企微单静态 Binding、PostgreSQL 持久消费者已有[本地及真实正常单聊证据](wecom-text-slice.md#验证结果)。企微失败与恢复行为仍以本地模拟平台测试为证据，当前不实现群聊、媒体、卡片和自动重发。四项 IM 设计要求见[收口记录](im-acceptance-closure.md)。飞书继续只保留第二类外部 IM 的差异设计。
