...
>
> 初始基线：2026-08-21；验收口径校准：2026-09-07。当前以[验收矩阵](acceptance.md)分别记录设计覆盖和实现证据。
...
| 初始上游基线 | `v1.11.2` |
| 开发语言 | Go 1.24.1 或更高版本 |
| 当前阶段 | 网页、企微持久文本链路、三阶段 OTel 和本地部署已验收；最终交付检查中，原通用 Channel 实验未纳入，见[交付入口](delivery.md) |
...
原题要求架构设计为主，并交付一份基于设计的 GitHub 实现代码；没有要求完整系统或逐功能全部编码。以下能力必须在方案中具体覆盖，参考实现选择能证明设计可落地的代表性链路：
...
设计验收可以通过模型、接口、伪代码、时序和风险说明完成；代码状态独立记录，不因设计完成而标记功能已实现。网页聊天与企业微信智能机器人长连接是已选演示方向，飞书实现暂缓但保留通道差异设计。选定的实现应可构建、可验证，未实现的生产能力不自动进入开发清单。
...
| 领域 | 基线选择 | 说明 |
| --- | --- | --- |
| Go | 1.24.1+ | 当前 `go.mod` 的真实工具链下限；由已固定的存储依赖传递抬升 |
...
| 热状态与协调 | Redis | 用于 Session 参考后端、限流、分布式租约和 Worker 唤醒；不作为 Inbox 持久去重事实源 |
...
以下约束描述目标架构及采用对应能力时的正确性要求，不表示本次必须实现所有组件。参考版本的实际能力与限制见[验收矩阵](acceptance.md)；已提供的隔离和安全保证仍须兑现。
...
| 范围标识 | 主要产出 | 当前验收方式 |
| --- | --- | --- |
| Phase 0 | 项目背景、架构、术语与验收矩阵 | 原题七项验收和八类交付物有证据入口 |
| Phase 1 | 参考执行链路 | 复核真实 Runner、Session、HTTP/SSE 与选定版本的运行证据 |
| Phase 2 | 多租户控制面与隔离 | 设计完整；已有创建、发布、路由和隔离实现按实际测试取证 |
| Phase 3 | 多节点与多后端 | 说明共享状态、并发、至少三类后端及迁移策略；已有代码提供补充证据 |
| Phase 4 | IM Channel | 至少两类差异设计，含微信或企微；真实收发按选定演示范围取证 |
| Phase 5 | 治理与可观测性 | 策略、指标、审计字段和 Trace 传播设计；实现边界如实列出 |
| Phase 6 | 故障恢复与生产部署 | 降级、容量估算、最小与生产部署设计，以及风险和缓解措施 |
| Phase 7 | 最终交付 | 完整设计材料、GitHub 参考实现、对应验证证据和明确限制 |

以上保留 Phase 标识以便追溯历史记录，不再作为全部功能依次编码的阶段门。生产风险至少列八项，并说明触发条件、影响、当前边界、监测或降级、生产缓解方案和残余风险；不要求消除所有风险。

## 11. 完成定义

设计完成按原题的具体模型、接口、时序、取舍和风险说明验收。功能实现完成则必须具有真实代码和对应验证，以下条件仅适用于纳入参考实现的功能：

- 存在可运行的真实实现，而非伪代码、空接口或仅有设计说明。
- 按风险覆盖关键正常路径、不变量和代表性失败路径，所选跨组件链路有可复现验证。
- 文档描述与当前行为一致，配置和运行步骤可以复现。
- 提供足以定位所选链路问题的证据；完整指标与 Trace 尚未实现时明确标注，不伪造可观测性。
- 明确租户隔离、权限、密钥、审计和失败处理行为。
- 记录构建和相关测试结果，使后续变更能够复核；原题未要求建设完整 CI 平台。
