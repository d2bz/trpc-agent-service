...
凭据身份预检、本地代码验收、真实长连接和下述正常单聊收发均已有证据。真实事件与成功回复证明本次消息权限及订阅已生效；不据此推断后台发布流程、持续在线或故障恢复保证。

## 真实单聊验证

2026-09-08 用户报告已发送后，只读核对独立 schema 的持久状态；运行二进制来自 `f6bb709`，使用已发布的真实模型配置。以下时间为 Asia/Shanghai：

| 受理时间 | 会话序号 | Run 状态/执行次数 | 回复状态/发送次数 | 平台确认时间 |
| --- | --- | --- | --- | --- |
| 16:01:41 | 1 | succeeded / 1 | sent / 1 | 16:01:47 |
| 16:02:43 | 2 | succeeded / 1 | sent / 1 | 16:02:46 |

数据库共 2 条 Inbox、2 条 Run、2 条 Outbox，属于同一 Principal、Session 和 Revision。两条回复均有非空平台消息 ID、`duplicate_risk=false` 且无执行/发送错误。验证过程未输出正文、外部账号或凭据；`sent` 表示飞书 API 已确认消息，不表示用户已读。这两轮证明正常接收、同会话续聊及最终回复链路；去重、失败和恢复仍以本地协议/集成测试为证据，不升级为真实平台故障验收。
