...
8. 连接重建后旧 ReplyTarget 无效，待发送回复记录为失败；不能宣称跨连接重放或跨重启最终送达。平台尚无已验证入站 ACK/补发保证，读取后落库前仍有丢失窗口。真实平台故障与补发行为尚未联调验证。
...
上述范围已完成本地验收，并补充一次真实账号正常单聊验证，证据如下；不继续扩展本切片功能。
...
2026-09-07 18:29:00（北京时间），用户在真实企业微信机器人单聊发送 `17 * 23` 测试。运行二进制来自已验收实现 `0378175`（与 `4121770` 同树），连接官方长连接地址及本地 PostgreSQL，使用 AIHubMix `gpt-4o-mini`、Revision `assistant-live-v1`；Bot 凭据只从本地进程配置读取，不写入验收记录。

实际数据库证据：Inbox 1 条；Run 为 `succeeded`、attempt 为 1、执行耗时 2526 ms；Outbox 为 `sent`、attempt 为 1、`duplicate_risk=false`，回复包含 `391`。`sent` 由同一回复请求的明确 `errcode=0` 回执确认，证明平台接受回复，不表示用户已读。此前独立订阅探测也已收到 `errcode=0`，探测退出后才启动唯一正式实例。

这次只新增正常真实单聊证据；重复投递、拒绝、断线和恢复仍以本地模拟平台测试为证据，不宣称完成生产环境验收。已启动即中断的 Run 明确失败而非重放，连接重建后旧回复记录为失败，两项残余边界保持不变。本次联调没有新增功能代码。
