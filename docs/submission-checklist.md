...
| 组织者要求的验收分支 | `feature/d2bz` |
| 方案版本 | `1.0-rc1` |
| 方案提交日期 | 2026-08-27 |
| 最终验收日期 | 2026-09-11 |

当前 GitHub 账号对题目源仓库没有推送权限。8 月 27 日从个人 Fork 的同名分支提交材料；最终验收从 `d2bz:feature/d2bz` 向 `liuzengh:main` 发起 PR。若组织者要求分支必须直接位于源仓库，需要先将 `d2bz` 加为协作者，再推送同名分支，不能通过改分支名绕过权限要求。

## 2. 方案材料

- [x] 2000–4000 字方案总稿：中文正文 2403 字。
- [x] 系统架构图：Mermaid 源码位于 `architecture.md`，CLI 11.12.0 渲染与视觉检查通过。
- [x] 企业微信核心时序图：Mermaid 源码位于 `sequence.md`，两张时序图均已渲染检查。
...
- [ ] 8 月 27 日提交 commit 已推送，且本地与远端 SHA 一致。
- [x] 工作区干净，无运行日志、PID、二进制、覆盖率文件或密钥进入 Git。
- [ ] 提交平台所需的仓库、分支、文档入口和演示说明已经填写。

## 5. 当前实现边界

截至 `1.0-rc1`，已实现最小 Runner 链路、Tenant/App/Revision 内存控制面、Admin API、动态 Runtime 路由、版本发布/回滚和多租户隔离测试。PostgreSQL、Redis 共享 Session、双 Worker、IM Adapter、治理、Telemetry 和生产部署仍在后续计划中。方案冻结不改变这些功能的 `planned/partial` 状态。
