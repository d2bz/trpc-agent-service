...
| I02 | OpenAI-compatible HTTP 服务 | `trpcservice/agent/agent.go`、`trpcservice/web/platform.go`、`cmd/trpc-service/main.go`：健康检查、非流式/SSE、优雅退出与超时后强制 `Close` | `go test ./trpcservice/web ./cmd/trpc-service`：健康、普通对话、SSE `[DONE]`、SSE 期间 Runtime 租约保持、Shutdown 失败后强制关闭 | partial |
