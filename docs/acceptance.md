...
| A19 | Plugin/Guardrail/Callback 租户治理 | [Tool 与 Policy Runtime](tool-policy.md)、[治理](solution.md#56-治理与安全) | 已实现静态 Tool Registry、Revision ToolRefs、Policy 白名单交集、未知/重复/越权 fail closed、Tool callback 审计和工具循环上限；租户 Policy entitlement、预算、审批、Guardrail 和动态扩展待实现 | partial |
...
| A22 | 审计字段完整 | [Audit 模型](data-model.md#36-inboxrunoutbox-与-audit)、[Tool 审计](tool-policy.md#3-执行与审计) | 已实现 Tool before/after 结构化事件，可信 RunContext 作用域、call ID、成功状态和耗时有字段与泄漏测试；持久化 Audit Store、查询、保留期、管理操作与全链路审计待实现 | partial |
...
- **Tool Policy 不是租户 entitlement。** Revision 当前可以引用进程内任何已注册 Policy；唯一内置 Policy 只开放两个无副作用工具。业务 Tool 上线前必须增加租户级 Policy 授权。4 轮工具循环上限也不是 Run deadline，既不限制慢模型/慢 Tool，也不限制单轮并行 Tool 数；审计只落结构化日志，不是持久化 Audit Store。详见 [Tool 与 Policy Runtime](tool-policy.md#5-明确限制)。
